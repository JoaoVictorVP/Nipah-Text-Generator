...
public class Trainer
{
    ...
    public void Train(Context ctx, string text, TrainerOptions options)
    {
        ...
        DoTrain(ctx, CollectionsMarshal.AsSpan(tokens), options, options.MaxDeep, null);
    }
    void DoTrain(Context ctx, ReadOnlySpan<Token> tokens, TrainerOptions options, int deepCounter, Neuron? carry)
    {
        ...
        {
            var iterator = ctx.GetOrCreateIteratorFor(carry);
            var layer = iterator.GetNext();
            layer.Reinforce(neuron, options.Bias);
        }
        DoTrain(ctx, tokens[1..], options, deepCounter - 1, neuron);
    }
}
